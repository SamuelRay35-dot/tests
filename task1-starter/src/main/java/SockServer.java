...
public class SockServer {
  ...
  static JSONObject concat(JSONObject req) {//errors: type was concat, testField only for string1, no try catch for string check
    ...
    try{
      ...
      res.put("ok", false);
      res.put("message", "Field string1/string2 needs to be of type: String");
    }
    return res;
  }
  ...
  static JSONObject currency(JSONObject req){
    ...
    if (!amountCheck.getBoolean("ok")) {
      return amountCheck;
    }

    JSONObject fromCheck = testField(req, "from");
    if (!fromCheck.getBoolean("ok")) {
      return fromCheck;
    }

    JSONObject toCheck = testField(req, "to");
    if (!toCheck.getBoolean("ok")) {
      return toCheck;
    }
